...
int main() {
    ...
    int N;
    getInput(N, "Вместимость рюкзака: ");

    int productCount;
    getInput(productCount, "Количество предметов: ");

    Product* products = new Product[productCount];
    for (int i = 0; i < productCount; ++i) {
        getInput(products[i].name, "Название: " + std::to_string(i + 1) + ": ");
        getInput(products[i].weight, "Вес: " + std::to_string(i + 1) + ": ");
        getInput(products[i].value, "Цена: " + std::to_string(i + 1) + ": ");
    }
